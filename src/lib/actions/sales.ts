...
export interface InvoiceLineItem {
  ...
  taxRate: string;
  taxType?: string; // GST, SRB, PRA, KPRA, BRA
  lineTotal: string;
}
...
export async function getInvoices(
  ...
  try {
    ...
    // Add search filter at SQL level
    if (searchQuery) {
      conditions.push(
        or(
    ...
    if (pagination) {
      ...
      const result = await baseQuery.limit(pagination.pageSize).offset(offset);
      return {
        success: true,
        data: result,
      ...
      await db.transaction(async (tx) => {
        for (const item of items) {
          ...
          await tx
            .update(products)
            .set({
              currentStock: sql`CAST(${products.currentStock} AS DECIMAL) + ${quantity}`,
            })
            .where(eq(products.id, item.productId));

          // Reverse warehouse stock
          if (invoice.warehouseId) {
            await updateWarehouseStock(tx, invoice.warehouseId, item.productId, quantity);
          }

          // Reverse batch stock
          if (item.batchId) {
            await updateBatchStock(tx, item.batchId, quantity);
          }

          // Read updated stock for running balance
          const [updatedProduct] = await tx
            .select({ currentStock: products.currentStock })
            .from(products)
            .where(eq(products.id, item.productId))
            .limit(1);
          const newRunningBalance = updatedProduct?.currentStock || "0";

          // Add reversal stock movement record
...
export async function getInvoiceStats() {
  try {
    ...
    const paidInvoices = allInvoices.filter(
      (inv) => inv.status === "paid" || inv.status === "approved",
    ).length;
  ...
  try {
    ...
    let result = await db
      .select({
        ...
        netAmount: quotations.netAmount,
...
export async function getQuotationById(quotationId: string) {
  try {
    ...
    const [quotation] = await db
      .select()
      .from(quotations)
      .where(and(eq(quotations.id, quotationId), eq(quotations.orgId, orgId)))
      .limit(1);
  ...
  try {
    ...
    await db
      ...
      .values({
        ...
        changes: JSON.stringify({ quotationNumber, netAmount }),
      });
...
export async function updateQuotation(
  ...
  try {
    ...
    if (data.shippingCharges !== undefined)
      updateData.shippingCharges = data.shippingCharges;
  ...
  try {
    ...
    const [newOrder] = await db
      ...
      .values({
        ...
        orderBooker: "",
        subject: quotation.subject || `From ${quotation.quotationNumber}`,
        reference: quotation.reference,
        orderDate: quotation.issueDate,
        deliveryDate: quotation.expiryDate,
        status: "draft",
    ...
    await db
      .update(quotations)
      .set({ status: "converted", isConverted: true })
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const result = await db
        ...
        .limit(1);
      let nextNumber = 1;
      if (result.length > 0 && result[0].deliveryNumber) {
        const match = result[0].deliveryNumber.match(/\d+$/);
        if (match) nextNumber = parseInt(match[0]) + 1;
      }
      const jitter = attempt > 0 ? Math.floor(Math.random() * (attempt + 1)) : 0;
      return `DN-${String(nextNumber + jitter).padStart(5, "0")}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `DN-${String(Date.now() % 100000).padStart(5, "0")}`;
}
  ...
  try {
    ...
    let result = await db
      .select({
        ...
        shippedVia: deliveryNotes.shippedVia,
        trackingNumber: deliveryNotes.trackingNumber,
        customer: { id: customers.id, name: customers.name },
      })
      .from(deliveryNotes)
  ...
  try {
    ...
    const [newDelivery] = await db
      ...
      .values({
        ...
        status: "pending",
        shippedVia: data.shippedVia || null,
...
export async function updateDeliveryStatus(
  ...
  try {
    ...
    const [updated] = await db
      ...
      .set(updateData)
      .where(
        and(eq(deliveryNotes.id, deliveryId), eq(deliveryNotes.orgId, orgId)),
      )
      .returning();
...
async function generateRecurringInvoiceNumber(orgId: string): Promise<string> {
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      const jitter = attempt > 0 ? Math.floor(Math.random() * (attempt + 1)) : 0;
      return `SL-${String(nextNumber + jitter).padStart(5, "0")}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `SL-${String(Date.now() % 100000).padStart(5, "0")}`;
}
...
export async function getRecurringInvoices(statusFilter?: string) {
  try {
    ...
    if (statusFilter && statusFilter !== "all")
      conditions.push(eq(recurringInvoices.status, statusFilter));
    const result = await db
      .select({
...
export async function createRecurringInvoice(data: RecurringInvoiceFormData) {
  try {
    const orgId = await getCurrentOrgId();
    if (!orgId) return { success: false, error: "No organization found" };
    if (!data.customerId || !data.templateName)
      return {
        success: false,
        error: "Customer and template name are required",
      };
...
export async function updateRecurringInvoice(
  ...
  try {
    ...
    const [updated] = await db
      ...
      .set(updateData)
      .where(
        and(eq(recurringInvoices.id, id), eq(recurringInvoices.orgId, orgId)),
      )
      .returning();
    if (!updated)
...
export async function generateRecurringInvoices() {
  try {
    ...
    const dueRecurring = await db
      ...
      .where(
        and(
          eq(recurringInvoices.orgId, orgId),
          eq(recurringInvoices.status, "active"),
        ),
      );
    let generated = 0;
    for (const recurring of dueRecurring) {
      if (!recurring.nextInvoiceDate || recurring.nextInvoiceDate > today)
        continue;
      ...
      const [newInvoice] = await db
        ...
        .values({
          ...
          subject: recurring.subject || recurring.templateName,
          issueDate: today,
          status: "draft",
          grossAmount: grossAmount.toFixed(2),
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const result = await db
        ...
        .from(salesReturns)
        .where(eq(salesReturns.orgId, orgId))
      ...
      const jitter = attempt > 0 ? Math.floor(Math.random() * (attempt + 1)) : 0;
      return `SR-${String(nextNumber + jitter).padStart(5, "0")}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `SR-${String(Date.now() % 100000).padStart(5, "0")}`;
}
...
export async function getSalesReturns(
  ...
  try {
    ...
    if (statusFilter && statusFilter !== "all")
      conditions.push(eq(salesReturns.status, statusFilter));
    let result = await db
      .select({
        id: salesReturns.id,
...
export async function approveSalesReturn(returnId: string) {
  try {
    ...
    const [salesReturn] = await db
      .select()
      .from(salesReturns)
      .where(and(eq(salesReturns.id, returnId), eq(salesReturns.orgId, orgId)))
      .limit(1);
    if (!salesReturn)
    ...
    await db.transaction(async (tx) => {
      // 1. Reverse stock for returned items
      for (const item of items) {
        if (item.productId) {
          ...
          if (product) {
            const newStock =
              parseFloat(String(product.currentStock || "0")) +
      ...
      // 2. Update invoice balance if linked
      if (salesReturn.invoiceId) {
        const [inv] = await tx
          .select()
          .from(invoices)
          .where(eq(invoices.id, salesReturn.invoiceId))
          .limit(1);
        if (inv) {
          const newBalance =
            parseFloat(inv.balanceAmount || "0") -
            parseFloat(salesReturn.refundAmount || "0");
          let newStatus:
            | "draft"
            | "pending"
            | "approved"
          ...
          else if (newBalance < parseFloat(inv.netAmount || "0"))
            newStatus = "partial";
          await tx
            .update(invoices)
...
async function generatePaymentNumber(orgId: string): Promise<string> {
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const result = await db
        .select({ paymentNumber: customerPayments.paymentNumber })
      ...
      const jitter = attempt > 0 ? Math.floor(Math.random() * (attempt + 1)) : 0;
      return `CP-${String(nextNumber + jitter).padStart(5, "0")}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `CP-${String(Date.now() % 100000).padStart(5, "0")}`;
}
...
export async function getCustomerOutstandingInvoices(customerId: string) {
  try {
    ...
    const result = await db
      ...
      .from(invoices)
      .where(
        and(eq(invoices.customerId, customerId), eq(invoices.orgId, orgId)),
      );
    return {
      success: true,
...
async function generateSettlementNumber(orgId: string): Promise<string> {
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const result = await db
        .select({ settlementNumber: settlements.settlementNumber })
        .from(settlements)
        .where(eq(settlements.orgId, orgId))
        .orderBy(desc(settlements.createdAt))
        .limit(1);
      let nextNumber = 1;
      if (result.length > 0 && result[0].settlementNumber) {
        const match = result[0].settlementNumber.match(/\d+$/);
        if (match) nextNumber = parseInt(match[0]) + 1;
      }
      const jitter = attempt > 0 ? Math.floor(Math.random() * (attempt + 1)) : 0;
      return `STL-${String(nextNumber + jitter).padStart(5, "0")}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `STL-${String(Date.now() % 100000).padStart(5, "0")}`;
}
...
export async function createCustomerSettlement(data: SettlementFormData) {
  try {
    ...
    const totalDiscount = data.documents.reduce(
      (sum, doc) => sum + parseFloat(doc.discountAmount || "0"),
      0,
    );
    const totalSettlement = data.documents.reduce(
  ...
  try {
    ...
    const [newOrder] = await db
      ...
      .values({
        ...
        subject: originalOrder.subject
          ? `[Copy] ${originalOrder.subject}`
          : null,
