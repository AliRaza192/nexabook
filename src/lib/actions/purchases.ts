...
async function generatePurchaseReturnNumber(orgId: string): Promise<string> {
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      return `PR-${String(nextNumber + jitter).padStart(5, '0')}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `PR-${String(Date.now() % 100000).padStart(5, '0')}`;
}
...
async function generateVendorPaymentNumber(orgId: string): Promise<string> {
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      const jitter = attempt > 0 ? Math.floor(Math.random() * (attempt + 1)) : 0;
      return `VP-${String(nextNumber + jitter).padStart(5, '0')}`;
    } catch {
      if (attempt < 4) await new Promise(r => setTimeout(r, Math.random() * 100 * (attempt + 1)));
    }
  }
  return `VP-${String(Date.now() % 100000).padStart(5, '0')}`;
}
