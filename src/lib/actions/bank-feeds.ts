...
/**
 * Encrypt sensitive fields in bank connection config before storing in DB.
 */
function encryptConfig(config: Record<string, string>): Record<string, string> {
  const encrypted: Record<string, string> = {};
  for (const [key, value] of Object.entries(config)) {
    // Encrypt keys that contain sensitive data
    if (["password", "token", "accessToken", "refreshToken", "secret", "apiKey", "credentials"].some(
      (sensitive) => key.toLowerCase().includes(sensitive)
    )) {
      encrypted[key] = isEncrypted(value) ? value : encryptToken(value);
    } else {
      encrypted[key] = value;
    }
  }
...
/**
 * Decrypt sensitive fields in bank connection config when reading from DB.
 */
...
export async function syncBankTransactions(connectionId: string) {
  try {
    ...
    // Decrypt config for provider use
    const decryptedConfig = decryptConfig(connection.config as Record<string, string> || {});
