...
export default function ReceivePaymentPage() {
  ...
  const loadOutstanding = async (custId: string) => {
    ...
    if (!custId) { setOutstanding([]); return; }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data: CustomerPaymentFormData = {
        customerId, paymentDate, paymentMethod, amount,
        reference: reference || undefined, notes: notes || undefined,
        allocations: allocs.length > 0 ? allocs : undefined
      };
      const res = await createCustomerPayment(data);
      if (res.success) { alert("Payment recorded successfully"); router.push("/sales/invoices"); }
    ...
    <div className="space-y-3 bg-gray-50 min-h-screen pb-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-gray-200 rounded-lg px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3"><Link href="/sales/invoices"><Button variant="outline" size="sm" className="h-8"><ArrowLeft className="h-4 w-4 mr-1" />Back</Button></Link><h1 className="text-lg font-bold text-gray-900 flex items-center gap-2"><DollarSign className="h-5 w-5 text-green-600" />Receive Payment</h1></div>
        </div>
      </motion.div>
            ...
            <Card className="border-gray-200 shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full"><thead><tr className="border-b border-gray-200 text-xs font-semibold text-gray-700 bg-gray-50">
                  <th className="text-left py-2 px-3">Invoice#</th><th className="text-left py-2 px-3">Issue Date</th><th className="text-left py-2 px-3">Due Date</th>
                  <th className="text-left py-2 px-3">Original</th><th className="text-left py-2 px-3">Balance</th><th className="text-left py-2 px-3">Allocate</th>
                </tr></thead><tbody>
                  {outstanding.map(inv => (
