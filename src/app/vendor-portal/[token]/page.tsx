...
import { getVendorPortalData } from "@/lib/actions/portal";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Building2, FileText, Phone, Mail, Loader2, AlertCircle, Download } from "lucide-react";
...
export default function VendorPortalPage() {
  ...
  useEffect(() => {
    if (!token) return;
    getVendorPortalData(token).then((res) => {
      setLoading(false);
  ...
  const totalDue = invoices
    ...
    .reduce((s: number, i: any) => s + parseFloat(i.netAmount || "0"), 0);
        ...
        <Card className="border-nexabook-200 shadow-sm">
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-nexabook-900">{vendor.orgName}</h1>
                <p className="text-sm text-nexabook-500 flex items-center gap-3 mt-1">
                  {vendor.orgPhone && <span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" />{vendor.orgPhone}</span>}
                  {vendor.orgEmail && <span className="flex items-center gap-1"><Mail className="h-3.5 w-3.5" />{vendor.orgEmail}</span>}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="border-nexabook-200">
            <CardContent className="p-5">
              ...
              <p className="text-xl font-bold text-nexabook-900 mt-1">{vendor.name}</p>
            </CardContent>
          </Card>
          <Card className="border-nexabook-200">
            <CardContent className="p-5">
              <p className="text-sm text-nexabook-500">Outstanding Balance</p>
              <p className="text-2xl font-bold text-orange-600 mt-1">Rs. {totalDue.toLocaleString("en-PK", { minimumFractionDigits: 2 })}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-nexabook-200 shadow-sm">
          <CardHeader className="pb-3 border-b border-nexabook-100">
            <CardTitle className="text-base font-semibold text-nexabook-900 flex items-center gap-2">
              ...
              Purchase Bills ({invoices.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {invoices.length === 0 ? (
              <div className="text-center py-10 text-nexabook-400 text-sm">No bills found</div>
            ) : (
              <div className="divide-y divide-nexabook-100">
                {invoices.map((inv: any) => (
                  <div key={inv.id} className="flex items-center justify-between px-5 py-3.5 hover:bg-nexabook-50 transition-colors">
                    <div>
                      <p className="text-sm font-medium text-nexabook-900">{inv.billNumber}</p>
