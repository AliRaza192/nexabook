...
export const invoices = pgTable('invoices', {
  ...
  uniqueInvoiceNumber: unique('uq_invoice_org_number').on(table.orgId, table.invoiceNumber),
}));
...
export const saleOrders = pgTable('sale_orders', {
  ...
  uniqueOrderNumber: unique('uq_sale_order_org_number').on(table.orgId, table.orderNumber),
}));
...
export const purchaseInvoices = pgTable('purchase_invoices', {
  ...
  uniqueBillNumber: unique('uq_purchase_invoice_org_number').on(table.orgId, table.billNumber),
}));
...
export const quotations = pgTable('quotations', {
  ...
  uniqueQuotationNumber: unique('uq_quotation_org_number').on(table.orgId, table.quotationNumber),
}));
...
export const purchaseOrders = pgTable('purchase_orders', {
  ...
  uniqueOrderNumber: unique('uq_purchase_order_org_number').on(table.orgId, table.orderNumber),
}));
...
export const goodReceivingNotes = pgTable('good_receiving_notes', {
  ...
  uniqueGrnNumber: unique('uq_grn_org_number').on(table.orgId, table.grnNumber),
}));
...
// Reconciliation Patterns (Smart Reconciliation - Pattern Learning)
export const reconciliationPatterns = pgTable('reconciliation_patterns', {
  id: uuid('id').primaryKey().defaultRandom(),
  orgId: uuid('org_id').references(() => organizations.id).notNull(),
  bankPattern: varchar('bank_pattern', { length: 500 }).notNull(), // e.g. "HBL FEE"
  bookPattern: varchar('book_pattern', { length: 500 }).notNull(), // e.g. "Bank Service Charge"
  matchCount: integer('match_count').notNull().default(1),
  confidence: decimal('confidence', { precision: 5, scale: 2 }).notNull().default('100'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});

// ==================== DIGITAL FTE PRODUCTS (MONETIZATION) ====================

// Digital FTE Products — sellable AI worker products
export const digitalFteProducts = pgTable('digital_fte_products', {
  id: uuid('id').primaryKey().defaultRandom(),
  slug: varchar('slug', { length: 100 }).notNull().unique(), // e.g. "accounting-fte"
  name: varchar('name', { length: 200 }).notNull(), // e.g. "Accounting FTE"
  description: text('description').notNull(),
  features: jsonb('features').notNull().default('[]'), // JSON array of feature strings
  priceMonthly: decimal('price_monthly', { precision: 10, scale: 2 }).notNull(), // USD/month
  priceYearly: decimal('price_yearly', { precision: 10, scale: 2 }), // USD/year (discounted)
  stripePriceIdMonthly: varchar('stripe_price_id_monthly', { length: 255 }), // Stripe Price ID for monthly
  stripePriceIdYearly: varchar('stripe_price_id_yearly', { length: 255 }), // Stripe Price ID for yearly
  category: varchar('category', { length: 50 }).notNull(), // "accounting", "tax", "inventory", "payroll", "crm"
  isActive: boolean('is_active').notNull().default(true),
...
// Organization FTE subscriptions — which FTEs an org has purchased
export const orgFteSubscriptions = pgTable('org_fte_subscriptions', {
  id: uuid('id').primaryKey().defaultRandom(),
  orgId: uuid('org_id').references(() => organizations.id).notNull(),
  fteProductId: uuid('fte_product_id').references(() => digitalFteProducts.id).notNull(),
  status: varchar('status', { length: 20 }).notNull().default('active'), // active, canceled, past_due, trial
  stripeSubscriptionId: varchar('stripe_subscription_id', { length: 255 }),
  stripeCustomerId: varchar('stripe_customer_id', { length: 255 }),
